...
const notesGet = async ( req, res = response ) => {
    try {
        ...
        if ( notes.length === 0 ) return res.status( 202 ).json( { errors: [ { msg: 'There are no notes from this user' } ] } );
...
const notesGetByID = async ( req, res = response ) => {
    try {
        ...
        if ( !note ) return res.status( 401 ).json( { errros: [ { msg: 'Note dosnt exist' } ] } );
